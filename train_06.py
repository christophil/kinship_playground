...
file_path = 'new facevgg.h5'


checkpoint = ModelCheckpoint(file_path, monitor='val_accuracy', verbose=1, save_best_only=False, mode='max')
...
history = model.fit_generator(generate(train, train_person_to_images_map, batch_size=16), use_multiprocessing=True,
                    validation_data=generate(val, val_person_to_images_map, batch_size=16), epochs=1, verbose=1,
                    workers = 4,callbacks=callbacks_list, steps_per_epoch=1, validation_steps=100)
